    ...
    <div className="relative pt-16 pb-12 px-5 rounded-b-[3rem] bg-gradient-to-br from-[#1A1A2E] via-[#6B3E26] to-[#1A1A2E] overflow-hidden -mx-4 -mt-4 sm:mx-0 sm:mt-0 shadow-2xl">
      ...
      <div className="relative z-10 flex flex-col items-center text-center space-y-6">
        <div className="flex flex-col items-center justify-center">
            {/* Inline SVG for ኑ with scaling */}
            <svg width="64" height="64" viewBox="0 0 512 512" className="mb-2">
                <style>{`@import url('https://fonts.googleapis.com/css2?family=Noto+Sans+Ethiopic:wght@900&display=swap');`}</style>
                <text x="256" y="380" fontFamily="'Noto Sans Ethiopic', sans-serif" fontSize="340" fontWeight="900" fill="#D4AF37" textAnchor="middle">ኑ</text>
            </svg>
...
function CategorySection() {
  const categories = [
    { icon: Car, label: "Pickup", href: "/transport" },
    { icon: BedDouble, label: "Stays", href: "/stays" },
    { icon: Map, label: "Tours", href: "/tours" },
    { icon: Navigation, label: "Transport", href: "/transport" },
    { icon: UtensilsCrossed, label: "Dining", href: "/dining" },
    { icon: Wine, label: "Nightlife", href: "/dining" },
  ];
      ...
      <div className="flex gap-4 overflow-x-auto no-scrollbar pb-4 px-2">
        {isLoading
          ...
          : data?.places?.map((place: PlaceData) => {
              const displayPrice = place.price_from;
                  ...
                  <div className="mt-4 flex items-center justify-between border-t border-gray-100 pt-3">
                      ...
                      {displayPrice ? (
                          <div className="text-right">
                              <span className="text-sm font-black text-[#1A1A2E]">{displayPrice.startsWith('$') || displayPrice.toLowerCase().endsWith('etb') ? displayPrice : `$${displayPrice}`}</span>
...
function AiRecommendations() {
    const plans = [
        { title: "The Ultimate Addis Weekender", desc: "A perfectly paced 2-day dive into the city's heart.", img: "https://images.unsplash.com/photo-1547471080-7fc2caa6f17f?auto=format&fit=crop&q=80&w=800" },
        { title: "Historic Lalibela Journey", desc: "Discover the architectural wonders of the north.", img: "https://images.unsplash.com/photo-1533228876829-65c94e7b5025?auto=format&fit=crop&q=80&w=800" },
        { title: "Omo Valley Adventure", desc: "Deep cultural immersion in the southern nations.", img: "https://images.unsplash.com/photo-1523805009056-1d1123ce3eb2?auto=format&fit=crop&q=80&w=800" }
    ];
