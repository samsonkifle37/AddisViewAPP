    ...
    const handleGenerate = async (searchQuery: string) => {
        ...
        try {
            ...
            const isValidPlace = (place: any) => {
                ...
                if (!place.name || place.name.trim() === "" || place.name.toUpperCase() === "HOME") return false;
                if (!place.type) return false;
                // Check for garbled location strings (e.g. Xa 7ח)
                const locationStr = `${place.city || ""} ${place.area || ""}`;
                if (locationStr.includes("7ח") || locationStr.includes("Xa")) return false;
                return true;
            };

            const cleanedDays = (res.days || []).map((day: any) => ({
                ...day,
                morning: isValidPlace(day.morning) ? day.morning : null,
                afternoon: isValidPlace(day.afternoon) ? day.afternoon : null,
                evening: isValidPlace(day.evening) ? day.evening : null,
            }));

            setResultDays(cleanedDays);
