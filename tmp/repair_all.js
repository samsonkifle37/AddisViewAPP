...
async function repair() {
    console.log("Starting full repair via API...");
    for (const [name, urls] of Object.entries(REAL_IMAGES)) {
        try {
            ...
            if (data.success) {
                console.log(`- REPAIRED ${name}: ${data.fixed} places updated.`);
            } else {
                console.log(`X FAILED ${name}: ${data.error}`);
            }
        } catch (e) {
            console.log(`X FAILED ${name}: ${e.message}`);
        }
    }
}
