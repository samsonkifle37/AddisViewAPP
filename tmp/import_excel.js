...
function mapType(cat, sub) {
    ...
    if (sub.includes('church') || sub.includes('cathedral') || sub.includes('monument')) return 'museum'; // fallback culture

    return 'tour'; // Generic fallback
}
    ...
    try {
        ...
        for (const p of places) {
            ...
            if (res.rows.length > 0) {
                // Update
                await pg.query(
                    `UPDATE "Place" SET type = $1, description = $2, location = $3, latitude = $4, longitude = $5, website = $6 WHERE id = $7`,
                    [type, description, location, lat, lng, website, res.rows[0].id]
                );
                placesUpdated++;
            } else {
                // Insert
                await pg.query(
                    `INSERT INTO "Place" (id, name, slug, type, description, location, latitude, longitude, website, "createdAt", "updatedAt") 
                     VALUES (gen_random_uuid(), $1, $2, $3, $4, $5, $6, $7, $8, NOW(), NOW())`,
                    [p.name, slug, type, description, location, lat, lng, website]
                );
                placesAdded++;
            }
        }
        ...
        if (toursSheet) {
            ...
            for (const t of tours) {
                ...
                const price = t.starting_price || null;
                const website = t.source_url || null;

                const res = await pg.query(`SELECT id FROM "Place" WHERE name = $1`, [name]);
                if (res.rows.length > 0) {
                    await pg.query(
                        `UPDATE "Place" SET description = $1, location = $2, price = $3, website = $4 WHERE id = $5`,
                        [description, location, price, website, res.rows[0].id]
                    );
                    toursUpdated++;
                } else {
                    await pg.query(
                        `INSERT INTO "Place" (id, name, slug, type, description, location, price, website, "createdAt", "updatedAt") 
                         VALUES (gen_random_uuid(), $1, $2, 'tour', $3, $4, $5, $6, NOW(), NOW())`,
                        [name, slug, description, location, price, website]
                    );
                    toursAdded++;
                }
            }
